...
const {
  getTemplates, createTemplate, updateTemplate, deleteTemplate, proxyPdf, republishTemplate,
} = require('../controllers/ClaimTemplateController');

const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024 } });

router.get('/',                protect, getTemplates);
router.get('/:id/pdf',         protect, proxyPdf);
router.post('/:id/republish',  protect, authorize('admin'), republishTemplate);
